...
namespace Base.Assignments
{
	...
	public class AssignmentConcurrent : IAssignment, IDisposable
	{
		...
		public void Start()
		{
			...
			if (assignments.Count > 0)
			{
				assignments.ForEach(task =>
				{
					if (task.Completed)
					{
						Debug.LogWarning("Task in Concurrent already completed.");
						lock (_assignments)
						{
		...
		public void Add(IAssignment assignment)
		{
			...
			if (_isStarted)
			{
				Debug.LogError("Can't add Assignment. Concurrent already started.");
				return;
			}

			if (Completed)
			{
				Debug.LogError("Concurrent already completed, added Assignment will have no effect.");
			}
